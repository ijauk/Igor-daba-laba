...
<form method="POST" action="{{ url('/jobpostings/update', $jobposting) }}">
    ...
    <div class="mb-3">
        ...
        <x-select.tom-select name="education_id" id="education_id" placeholder="Traži i odaberi stupanj obrazovanja…"
            :endpoint="url('/api/educations')" value-field="id" label-field="text" search-field="text"
            :min-input-length="0" :max-options="50" dropdown-parent="body"
            :selected="$selectedEducation?->id" :options="$selectedEducation ? [$selectedEducation->id => $selectedEducation->label] : []" />
        @error('education_id')
    ...
    <div class="mb-3">
        ...
        <x-select.tom-select name=" job_position_id" id="job_position_id" placeholder="Traži i odaberi poziciju…"
            :endpoint="url('/api/job-positions')"
